...
The main difference between both of them that `standalone` controller is an independendent controller and does not require an [AscModule](../api-reference/modules.md) to work and host HTTP endpoints.

## Standalone Controllers
Standalone controllers don't require an [AscModule](../api-reference/modules.md) to access dependency injection scope. You can directly import modules or define and use providers right in the controller itself by managing them in the `@Controller` decorator itself
...
As same as [AscModule](../api-reference/modules.md) has parameters that manage it's dependency injection, a `standalone` controller has it too.
...
!!! tip
    Read more about Dependency Injection in AscModules at the [AscModule Guide Page](../api-reference/modules.md)


## Non-Standalone Controllers
This type of controllers require a parent [AscModule](../api-reference/modules.md) to define them as their declaration. As a non-standalone controller will be defined in `declarations` of an [`AscModule`](../api-reference/modules.md) it becomes the controller of that module which declared it in their `declarations`. It also becomes consumer of the module as it can access it's dependency injection scope and import available dependencies to that module
...
Now, `SupplierController` is controller and subject of an [AscModule](../api-reference/modules.md) `SupplierModule`. `SupplierController` can access DI scope of `SupplierModule` and use it's defined and imported providers
