...
You can also specify where to provide this service by specifying `--module`. Let's imagine we have an [AscModule](../api-reference/modules.md) in `example_module.py`:
```console
...
=== "With `--module` flag"
    ...
    and [AscModule](../api-reference/modules.md) in `src/examples/example_module.py`
