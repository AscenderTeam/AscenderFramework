...
class ModuleEditor(SchematicsEditor):
    ...
    def regex_processing(self, file_contents: str):
        ...
        final_imports = list(
            set(
                self.imports
                + [imp.strip() for imp in existing_imports[0].split(",") if imp.strip()]
            )
        )
        final_providers = list(
            set(
                self.providers
                + [
                    provider.strip()
                    for provider in existing_providers[0].split(",")
                    if provider.strip()
                ]
            )
        )
        final_declarations = list(
            set(
                ...
                + [
                    declaration.strip()
                    for declaration in existing_declarations[0].split(",")
        ...
        for namespace, package in self.package_imports.items():
            if (
                ...
                and file_contents.find(package) != -1
            ):
    ...
    def post_processing(self, regex_values: dict[str, Any] | list[Any]):
        return {"package_imports": self.package_imports.items(), **regex_values}
    ...
    def invoke(self):
        ...
        (
            ...
            final_providers,
            lower_section,
        ) = self.regex_processing(file_contents)
