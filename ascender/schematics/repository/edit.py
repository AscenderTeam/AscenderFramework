...
class RepositoryEditor(SchematicsEditor):
    def __init__(
        self, name: str, entity_imports: dict[str, type[E]], orm_mode: ORMEnum
    ):
        ...
        self.work_path = os.path.abspath(
            os.path.normpath(f"{self.path_config.source}/{name.lower()}_repository.py")
        )
    ...
    def regex_processing(self, file_contents: str):
        ...
        for namespace, entity_class in self.entity_imports.items():
            if (
                file_contents.find(namespace) != -1
                and file_contents.find(entity_class.__name__) != -1
            ):
                raise ValueError(
                    f"Import of entity {entity_class.__name__} from {namespace} already exists, please remove it to generate repository entities properly!"
                )
