...
from reactivex import Observable, Observer, abc, create
...
class HTTPClient:
    def __init__(self, base_url: str, transport: AscHTTPTransport, **client_configs):
        self.client = AsyncClient(
            base_url=base_url, transport=transport, **client_configs
        )

    async def get(
        self, _resp: type[T] | T = dict, *, url: str, options: HTTPOptions | None = None
    ) -> T:
    ...
    def stream(
        ...
        *,
        ...
        return create(
            cast(abc.Subscription[T], self.__handle_streaming(_resp, response=resposne))
        )

    def __prepare_request_body(self, content: Any | BaseModel | None):
        if not content:
            return {}
    ...
    def __handle_streaming(
        self, _resp: type[T] | T = dict, *, response: _GeneratorContextManager[Response]
    ):
    ...
    def __handle_response(self, _resp: type[T] | T = dict, *, response: Response):
        ...
        if issubclass(cast(type[T], _resp), BaseModel):
            _outload = cast(BaseModel, _resp).model_validate(response.json())
            return _outload
