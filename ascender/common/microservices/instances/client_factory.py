...
from ascender.common.microservices.types.transport import (
    TRANSPORT_PROXY_MAPPING, Transports)
from ascender.core.applications.application import Application
from ascender.core.di.injectfn import inject


class ClientProxyFactory:
    def __init__(self, instance: TransportInstance, application: Application):
        self.instance = instance
        self.application = application

    def as_provider(self):
        proxy_instance = TRANSPORT_PROXY_MAPPING[
            self.instance.transport["transport"].value
        ]
        instance = proxy_instance(
            self.instance.event_bus, self.instance.transport["options"], self.instance
        )
    ...
    @staticmethod
    def create(
        transport: Transports | type[ClientProxy], options: dict[str, Any] = {}
    ) -> ClientProxy:
        event_bus = inject("TRANSPORT_EVENTBUS")
        application = inject(Application)

        if isinstance(transport, Transports):
            proxy_instance = TRANSPORT_PROXY_MAPPING[transport.value](
                event_bus, options
            )
