...
class TCPClient(ClientProxy):
    def __init__(self, event_bus, configs: dict = {}, instance=None):
        ...
        if instance is not None:
            raise ValueError(
                "TCP transport doesn't support initiating with client proxy!"
            )
    ...
    async def send_as_observable(
        ...
        observable = await self.rpc_transport.send_nack_request(
            pattern=pattern, data=data, timeout=timeout
        )
        return observable.pipe(ops.map(lambda res: validate_python(res, response_type)))
