...
from ascender.common.microservices.abc.client_proxy import (ClientProxy, T,
                                                            Undefined)
...
class KafkaClient(ClientProxy):
    ...
    async def send(
        ...
        response = await self.rpc_transport.send_request(
            pattern=pattern, data=data, timeout=timeout
        )
    ...
    async def send_as_observable(
        ...
        response = await self.rpc_transport.send_nack_request(
            pattern=pattern, data=data, timeout=timeout
        )

        return response.pipe(ops.map(lambda res: validate_python(res, response_type)))

    async def emit(
        self, pattern: str, data: Any | BaseDTO | BaseResponse | None = None, **kwargs
    ):
        return await self.event_transport.send_event(
            pattern=pattern, data=data, **kwargs
        )
