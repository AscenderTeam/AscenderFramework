...
from ascender.common.microservices.instances.kafka.metadata import \
    KafkaMetadata
...
if TYPE_CHECKING:
    from ascender.common.microservices.instances.kafka.client import \
        KafkaClient
    from ascender.common.microservices.instances.kafka.context import \
        KafkaContext
...
class KafkaRPCTransport(RPCTransport):
    ...
    async def send_nack_request(self, pattern, data, timeout):
        ...
        # Send request
        await self.transport.producer.send(
            pattern, value=data, headers=[("correlationId", correlation_id.encode())]
        )
    ...
    async def defer(self, pattern: str, correlation_id: str):
        ...
        """
        self.logger.debug(
            f"[yellow] ASCENDER MICROSERVICES [/] | Requesting consumer side to defer response, and prolong the timeout at the pattern {pattern}"
        )
        await self.transport.producer.send(
            pattern,
            value=b"defer",
            headers=[("correlationId", f"defer-{correlation_id}".encode())],
        )
    ...
    async def send_response(self, pattern, correlation_id, response):
        ...
        await self.transport.producer.send(
            pattern, response, headers=[("correlationId", correlation_id.encode())]
        )
    ...
    async def raise_exception(self, pattern, correlation_id, exception):
        ...
        await self.transport.producer.send(
            pattern,
            json.dumps(exception.to_dict()).encode(),
            headers=[("correlationId", correlation_id.encode())],
        )
    ...
    async def process_response(self, correlation_id, response, **kwargs):
        ...
        self.response_subject.on_next(
            (correlation_id, decode_message(response), kwargs)
        )

    async def listen_for_requests(
        self, context: "KafkaContext", data: Any, metadata: KafkaMetadata
    ):
