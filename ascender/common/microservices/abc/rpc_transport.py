...
from ascender.common.microservices.types.consumer_metadata import \
    ConsumerMetadata
...
class RPCTransport(ABC):
    ...
    async def send_request(
        self, pattern: str, data: dict | BaseResponse | BaseDTO, timeout: float = 4000
    ...
    async def listen_for_requests(
        self, context: "BaseContext", data: Any, metadata: ConsumerMetadata
    ) -> None:
        """
    ...
    async def send_response(
        self, pattern: str, correlation_id: str, response: bytes
    ) -> None:
    ...
    async def raise_exception(
        self, pattern: str, correlation_id: str, exception: RPCException
    ):
        """
