...
from ascender.common.microservices.instances.client_factory import \
    ClientProxyFactory
...
def provideMicroservices(options: list[MicroserviceOptions]) -> list[Provider]:
    transports: list[Provider] = [
        {
            ...
            "use_factory": lambda event_bus, token=t["token"], options=t[
                "options"
            ]: TransportInstance(token, options, event_bus),
    ...
    clients: list[Provider] = [
        {
            "provide": t["token"],
            "use_factory": lambda instance, app: ClientProxyFactory(
                instance, app
            ).as_provider(),
            "deps": [f"TRANSPORT_INSTANCE_{t['token']}", Application],
        }
        for t in options
        if t.get("with_client_proxy", False)
    ]

    return [
        {"provide": "TRANSPORT_EVENTBUS", "use_class": SubscriptionEventBus},
        transports,
        clients,
        {
