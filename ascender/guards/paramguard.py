...
class ParamGuard:
    ...
    def _define_dependencies(self, executable: Callable[..., None]):
        ...
        methods = [
            getattr(getattr(self, method), "_name", method)
            for method in dir(self)
            if callable(getattr(self, method)) and method.endswith("_guard")
        ]
            ...
            if name + "_guard" in methods and param.default == Parameter.empty:
                new_parameters.append(
                    Parameter(
                        ...
                        default=Depends(getattr(self, f"{name}_guard")),
                    )
                )
            else:
    ...
    def __call__(self, executable: Callable[..., Any]):
        ...
        if hasattr(_updatedfunc, "__cmetadata__"):
            _updatedfunc.__cmetadata__["dependencies"] = [
                *executable.__cmetadata__.get("dependencies", []),
