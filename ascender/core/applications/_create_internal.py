...
def cli_factory(cli_settings: Sequence[BaseCLI | GenericCLI]):
    ...
    for cli_config in cli_settings:
        if isinstance(cli_config, BaseCLI):
            _cli.register_base(
                cli_config.__class__.__name__.removesuffix("CLI").lower(), cli_config
            )  # Use name of class as first argument of command
...
def createInternalApplication() -> CLI:
    ...
    internal_providers: list[Provider] = [
        ...
        provideCLI(NewCLI),
        provideCLI(RunCLI),
        {"provide": CLI, "use_factory": cli_factory, "deps": ["CLI_INTERFACE"]},
    ]
