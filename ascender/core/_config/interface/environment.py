...
class PathsConfig(BaseModel):
    ...
    root_imports: bool = Field(
        False, description="Explicitly treat `source` imports as python root imports."
    )
    output: str = Field(..., description="Path to the build output directory.")
    static: Optional[str] = Field(
        None, description="Path to the static files directory."
    )
...
class EnvironmentConfig(BaseModel):
    ...
    logging: Literal["debug", "info", "warn", "error", "critical"] = Field(
        ..., description="Logging level."
    )
    build: Optional[BuildConfig] = Field(
        None, description="Build-specific settings for this environment."
    )
...
class EnvironmentsConfig(BaseModel):
    ...
    environments: dict[
        Literal["development", "production", "test"], EnvironmentConfig
    ] = Field(..., description="Environment-specific configurations.")
