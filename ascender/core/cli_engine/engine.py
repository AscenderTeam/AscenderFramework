...
from ascender.core.cli_engine.types.command_metadata import (
    BasicMetadata_,
    GenericMetadata_,
)
from ascender.core.cli_engine.utils.argparser import RichArgumentParser
from ascender.core.cli_engine.utils.valiidator import (
    call_parsed_method,
    set_parsed_attr,
)
...
class CLIEngine:
    ...
    def process_commands(self, commands: Sequence[BasicCLI | GenericCLI]) -> None:
        ...
        for command_instance in commands:
            command_info: BasicMetadata_ | GenericMetadata_ | None = getattr(
                command_instance, "__asc_command__", None
            )
            if not command_info:
                continue
            if isinstance(command_info, BasicMetadata_):
                # BasicCLI command
                command_name = (
                    command_info.name or command_instance.__class__.__name__.lower()
                )
                ...
                aliases = (
                    command_info.additional.get("aliases")
                    if hasattr(command_info, "additional")
                    else None
                )
                if aliases:
    ...
    def execute_command(
        self, command_instance: Any, arguments: Mapping[str, Any]
    ) -> Any:
        """
        ...
        if isinstance(command_instance, BasicCLI):
            ...
            if not selected:
                console = Console()
                available = (
                    ", ".join(sorted(h.names[0] for h in command_info.parameters))
                    or "(no commands registered)"
                )
            ...
            for h in command_info.parameters:
                if selected in getattr(h, "names", []) or selected == getattr(
                    h, "_functionname", None
                ):
                    handler_info = h
                    break
            ...
            subcommand_args = {
                k: v for k, v in arguments.items() if k in handler_info.parameters
            }
    ...
    def parse_and_execute(self, argv: list[str] | None = None) -> Any:
        ...
        if getattr(args, "command", None) is None:
            console = Console()
            available = (
                ", ".join(sorted(self.command_map.keys())) or "(no commands registered)"
            )
            panel_text = Text.assemble(
                ("No command provided.\n\n", "bold white"),
        ...
        if not command:
            ...
            console.print(
                Panel(
                    Text(f"Unknown command: {args.command}", style="bold white on red"),
                    title="Error",
                    border_style="red",
                )
            )
