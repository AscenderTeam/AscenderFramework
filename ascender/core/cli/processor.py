...
class CLI:
    ...
    def __init__(
        ...
        *,
        ...
        base_clis: list[tuple[str, BaseCLI]] = [],
        callback: Optional[Callable[[ContextApplication], None]] = None,
    ) -> None:
    ...
    def register_base(self, name: str, base_cli: BaseCLI) -> None:
        ...
        if base_cli in self.base_clis:
            raise ValueError(
                f"BaseCLI Instance `{base_cli.__class__.__name__}` is already registered"
            )

        if not isinstance(base_cli, BaseCLI):
            raise ValueError(
                f"Instance `{base_cli.__class__.__name__}` is not a BaseCLI instance"
            )
    ...
    def load_basic(self, main_group: click.Group) -> None:
        for name, base_cli in self.base_clis:
            command = LoaderBaseCLI(
                base_cli, self.application, name, lambda: None
            ).run()
            main_group.add_command(command)

    def load_generic(self, main_group: click.Group) -> None:
        for generic_cli in self.generic_clis:
            groups = GenericLoader(
                generic_cli, self.application, callback=self.callback
            )
