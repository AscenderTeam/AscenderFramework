...
class GenericLoader:
    ...
    def _as_command(
        self, name: str, method_name: str, arguments: list[ArgumentsFormat], **metadata
    ) -> None:
        command = RichCommand(
            name,
            context_settings=self.cli.configs,
            callback=self.execute_cli(method_name),
            **metadata,
        )
