...
class LoaderBaseCLI:
        ...
        for index, argument in enumerate(arguments):
            ...
            if argument["is_ourobj"]:
                arguments[index] = {
                    "argument": (
                        argument["argument"]
                        if not isinstance(argument["value"], ArgumentCMD)
                        or not argument["value"].name
                        else argument["value"].name
                    ),
                    "type": (
                        argument["value"].type
                        if argument["value"].type is not None
                        else argument["type"]
                    ),
    ...
    def _as_command(self) -> None:
        ...
        command = click.RichCommand(
            self.name, context_settings=self.cli._config, callback=self.execute_cli
        )
                ...
                if arg["is_arg"]:
                    command.params.append(
                        click.Argument(
                            [arg["argument"]],
                            type=arg["type"],
                            default=arg["value"].default,
                            required=arg["value"].required,
                            **arg["value"].additional_kwargs,
                        )
                    )
                else:
