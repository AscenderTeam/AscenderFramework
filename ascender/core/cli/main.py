...
from ascender.core.errors.base import (DeclaredBaseCliIsNotDefined,
                ...
                result.append(
                    {
                        ...
                        "value": current_value,
                        "is_ourobj": bool(
                            type(current_value) in [ArgumentCMD, OptionCMD]
                        ),
                    }
                )
...
class GenericCLI:
    ...
    def get_method_arguments(
        self, _method: Callable, ignore: list[str]
    ) -> list[ArgumentsFormat]:
        result: list[ArgumentsFormat] = []
        ...
        for name, param in signature.parameters.items():
            ...
            if param_type == bool and type(default_value) not in [
                OptionCMD,
                ArgumentCMD,
            ]:
                default_value = False
            result.append(
                {
                    ...
                    "type": param_type,
                    "value": default_value,
                    "is_ourobj": bool(type(default_value) in [ArgumentCMD, OptionCMD]),
                }
            )
        return result
                ...
                for argument in arguments:
                    ...
                    if f"--{argument['argument'].lower()}" in argv:
                        ...
                        if argument["type"] == list:
                            cmd_argument = argv[
                                argv.index(f"--{argument['argument'].lower()}") + 1
                            ].split(",")
    ...
    def load_genericcli(self):
        ...
        for cli in cli_list:
            ...
            for method in methods:
                try:
                    if (
                        "".join(argv[1:]).find(method["name"].lower().replace("_", ""))
                        != -1
                    ):
                        # if argv[2] in ["-h", "--help"]:
                        #     cprint(method)
                        ...
                        for argument in arguments:
                            ...
                            if f"--{argument['argument'].lower()}" in argv:
                                try:
                                    ...
                                    cmd_argument = argv[
                                        argv.index(f"--{argument['argument'].lower()}")
                                        + 1
                                    ]
                                except IndexError:
                                    raise IncorrectCommandArgument(
                        ...
                        try:
                            ...
                            func(
                                ctx=ContextApplication(application=self.application),
                                **args,
                            )

                        except TypeError as ex:
                            _req_attr = (
                                ex.args[0].split(" ")[-1].lower().replace("'", "")
                            )
                            raise IncorrectCommandArgument(
...
"""
        ...
        for cli in self.generic_cli_list:
            ...
            for command in commands:
                commands_output += (
                    f"    [bold green]{command['name'].replace('_', ' ')}[/bold green] "
                )
                for name, arg_type in self.process_arguments(command["args"]):
                    arguments_output += (
                        f"--[bold yellow]{name}[/bold yellow] [{arg_type}] "
                    )
