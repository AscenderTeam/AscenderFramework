...
def is_static_class_provider(provider: Provider) -> bool:
    ...
    if not use_class_attr:
        return (
            bool(provider.get("provide", None))
            and bool(provider.get("deps", None))
            and not is_factory_provider(provider)
        )
...
def is_type_provider(provider: Provider) -> bool:
    return not (
        is_factory_provider(provider)
        and is_static_class_provider(provider)
        and is_value_provider(provider)
