...
from typing import (Any, Callable, ForwardRef, Mapping, MutableMapping,
                    MutableSequence, Sequence, Set, TypeVar, cast, overload)

from ascender.core.di.abc.base_injector import Injector
from ascender.core.di.interface.consts import (CIRCULAR, RAISE_NOT_FOUND,
                                               CyclicDependency)
...
from ascender.core.di.utils.forward_ref import (is_forward_ref,
                                                resolve_dep_forward_ref)
from ascender.core.di.utils.injection_def import injection_def
from ascender.core.di.utils.providers import (for_each_provider,
                                              is_factory_provider,
...
class AscenderInjector(Injector):
    ...
    def __init__(
        self, providers: Sequence[Provider], parent: AscenderInjector | None = None
    ):
    ...
    def get(
        ...
        return self.supply(token, not_found_value=not_found_value, options=options)
    ...
    def __process_provider(self, provider: Provider):
        ...
        provider_token = (
            provider if isinstance(provider, type) else provider.get("provide")
        )
    ...
    def __provide_to_factory(self, provider: Provider):
        ...
        if is_type_provider(provider):
            if isinstance(provider, ForwardRef):
                _resolved = resolve_dep_forward_ref(
                    provider,
                    [d for d in self.dependencies.keys() if isinstance(d, type)],
                )
        ...
        if is_value_provider(provider):
            ...
            factory = lambda: factory_provider["use_factory"](
                *self.__inject_args(tokens=factory_provider.get("deps", []))
            )

        elif is_static_class_provider(provider):
            static_class_provider = cast(StaticClassProvider, provider)
            class_ref = static_class_provider.get(
                "use_class", static_class_provider["provide"]
            )
            if not isinstance(class_ref, type):
                raise TypeError(
                    "Class provider has string as provider and lacks of `use_class` parameter"
                )
            if static_class_provider.get("deps", None):
                factory = lambda: class_ref(
                    self.__inject_args(static_class_provider["deps"])
                )
            else:
    ...
    def get_factory_def(
        self, token: type[Any] | str, self_only: bool = False, skip_self: bool = False
    ):
        ...
        _injectable_value = self.dependencies[
            token
        ]  # Returns set even if token was not found, it will return empty set if not found
    ...
    def supply(
        ...
        """
        _deps = self.get_factory_def(
            token, options.get("only_self", False), options.get("skip_self", False)
        )
            ...
            for dep in _deps:
                if dep.value is CIRCULAR:
                    raise CyclicDependency(
                        f"Circular dependency detected for token: {token}"
                    )
    ...
    def __inject_args(self, tokens: Sequence[type[Any] | str]) -> Sequence[Any]:
        ...
        for token in tokens:
            if is_forward_ref(token):
                try:
                    token = resolve_dep_forward_ref(
                        cast(ForwardRef | str, token), type_records
                    )
                except TypeError:
                    pass
    ...
    def __inject_kargs(
        self, tokens: Mapping[str, type[Any] | str]
    ) -> Mapping[str, Any]:
        """
