...
def unwrap_module_controller(
    module: type[AscModuleRef], target: type[ControllerRef] | None = None
):
    ...
    if hasattr(module.__asc_module__, "consumers"):
        for consumer in module.__asc_module__.consumers:  # type: ignore (behaviour of type checker is unpredictable here)
            ...
            if (
                hasattr(consumer, "__controller__")
                and consumer in module.__asc_module__.exports
            ):
                controller = consumer
                break
        ...
        if not controller:
            raise ValueError(
                f"AscModule {module.__name__} doesn't have any controllers declared!"
            )
