...
def handle_module_consumers(
    loaded_module: type[AscModuleRef], injector: AscenderInjector
) -> list[RouterNode]:
    """
...
def build_router_node(
    consumer: type[ControllerRef],
    parent_module: type[AscModuleRef] | None,
    ...
    pathname = (
        consumer.__controller__.name
        if consumer.__controller__.name
        else consumer.__name__.removesuffix("Controller").lower()
    )
    pathname = pathname.strip("/")
    suffix = consumer.__controller__.suffix.strip("/")
    tags = (
        consumer.__controller__.tags
        if consumer.__controller__.tags
        else [consumer.__name__.removesuffix("Controller")]
    )
