...
class NewService(Service):
    ...
    def initialize_project(
        ...
        _path = os.path.abspath(
            relpath.format(project_name=name.strip().replace(" ", ""))
        )
        ...
        self.create_generator_service.generate_module(f"controllers/controllers")

    def initialize_packages(self, project_name: str):
        ...
        subprocess.run(
            "poetry config virtualenvs.create true && poetry add ascender-framework",
            shell=True,
            check=True,
        )

    def create_project_files(
        self, path: os.PathLike | str, orm_mode: ORMEnum, standalone: bool
    ):
