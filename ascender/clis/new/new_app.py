...
class NewCLI(BaseCLI):
    project_name: str = OptionCMD(
        "--name", "--project-name", "-n", required=True, ctype=str
    )
    orm_mode: str = OptionCMD(
        ...
        default="sqlalchemy",
        required=False,
        ctype=click.Choice(["tortoise", "sqlalchemy"]),
    )
    standalone: bool = OptionCMD(
        "--no-standalone", default=False, is_flag=True, required=False
    )
    relpath: str = OptionCMD("--relpath", default="{project_name}", required=False)

    def __init__(self, new_service: NewService):
        self.new_service = new_service

    def callback(self, ctx: ContextApplication):
        return self.new_service.initialize_project(
            ctx,
            self.project_name,
            self.relpath,
            ORMEnum(self.orm_mode),
            self.standalone,
        )
