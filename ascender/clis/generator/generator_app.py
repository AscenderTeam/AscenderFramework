...
from ascender.clis.generator.create_generator_service import \
...
class GeneratorCLI(GenericCLI):
    ...
    help: str = (
        "Generates project components such as services, controllers, guards, CLIs, and modules."
    )
    ...
    def generate_controller(
        ...
        name: str,
        standalone: bool = OptionCMD(
            "--no-standalone", default=False, is_flag=True, required=False
        ),
        ...
        if name == "unicorn":
            ...
            ctx.console_print(
                "[red]Error:[/] Unicorns are mythical. Try generating a 'real' controller instead."
            )
            return

        module_update, file_info = self.create_generator_service.generate_controller(
            name, standalone, prefix, suffix, module
        )
        ctx.console_print(
            f"[green]{file_info['schematic_type']}[/green] {file_info['file_path']} [cyan]({file_info['write_size']} bytes)[/cyan]"
        )
        if module_update:
            ctx.console_print(
                f"[yellow]{module_update['schematic_type']}[/yellow] {module_update['file_path']} [cyan]({module_update['write_size']} bytes)[/cyan]"
            )
    ...
    def generate_service(
        ...
        module_update, file_info = self.create_generator_service.generate_service(
            name, module
        )
        ctx.console_print(
            f"[green]{file_info['schematic_type']}[/green] {file_info['file_path']} [cyan]({file_info['write_size']} bytes)[/cyan]"
    ...
    def generate_module(
        ...
        module_update, file_info = self.create_generator_service.generate_module(
            name, module
        )
        ctx.console_print(
            f"[green]{file_info['schematic_type']}[/green] {file_info['file_path']} [cyan]({file_info['write_size']} bytes)[/cyan]"
    ...
    def generate_repository(
        ...
        ctx: ContextApplication,
        name: str,
        entities: list[str] = OptionCMD(
            "-e", multiple=True, ctype=str, default=(), required=False
        ),
        orm_mode: str = OptionCMD(
            "--orm", "-om", ctype=click.Choice([e.value for e in ORMEnum])
        ),
        module: str | None = OptionCMD("-m", default=None, ctype=str, required=False),
    ):
        repository = self.create_generator_service.generate_repository(
            name, entities=entities, orm_mode=ORMEnum(orm_mode), module=module
        )
        if isinstance(repository, dict):
            return ctx.console_print(
                f"[yellow]{repository['schematic_type']}[/yellow] {repository['file_path']} [cyan]({repository['write_size']} bytes)[/cyan]"
            )
    ...
    def generate_guard(
        ...
        ctx: ContextApplication,
        name: str,
        guard_type: Literal["single", "parametrized"] = OptionCMD(
            "--guard-type",
            "-gt",
            ctype=click.Choice(["single", "parametrized"]),
            default="single",
            required=False,
        ),
        ...
        if guard_update:
            ctx.console_print(
                f"[yellow]{guard_update['schematic_type']}[/yellow] {guard_update['file_path']} [cyan]({guard_update['write_size']} bytes)[/cyan]"
            )
