...
class TestAscModule(TestCase):
    ...
    def test_module_load(self):
        try:
            ...
            RuntimeError
        ):  # In case if one of tests loaded module we should ignore `module already loaded` exeption
            first_module = FirstModule

        self.assertTrue(
            hasattr(first_module, "_injector"),
            "Loaded module lacks of injector, which means module load failed!",
        )
        self.assertIsInstance(
            first_module._injector,
            AscenderInjector,
    ...
    def test_encapsulation(self):
        ...
        with self.assertRaises(
            NoneInjectorException,
            msg="Encapsulation failed, `FirstModule` let `SecondModule` to import it's tokens without `exports` permission",
        ):
            second_module._injector.get(FirstService)

    def test_imports(self):
        third_module = load_module(ThirdModule)
        example_service = third_module._injector.get(
            ExampleService
        )  # Importing provider which's not defined in `ThridModule` but was defined & exported in `SecondModule`
