...
class TestForwardRef(TestCase):
    def test_is_forward_ref(self):
        ...
        self.assertTrue(
            _forwardref,
            "`ExampleService` should be forward ref as specified in ForwardRef or str formats!",
        )

        _wrong_forwardref = is_forward_ref(Application)
        self.assertFalse(
            _wrong_forwardref,
            "Application is type[Application] and can't be forward reference as is's original type itself!",
        )
    ...
    def test_dependency_forwardref(self):
        _resolved_string_reference = resolve_dep_forward_ref(
            "ExampleService", [ExampleService]
        )
        self.assertTrue(
            issubclass(_resolved_string_reference, ExampleService),
            "Resolved string is not Example Service. How odd...",
        )

        _resolved_forward_ref = resolve_dep_forward_ref(
            ForwardRef("ExampleService"), [ExampleService]
        )
        self.assertTrue(
            issubclass(_resolved_forward_ref, ExampleService),
            "Resolved forward reference is not `ExampleService`. How odd...",
        )
