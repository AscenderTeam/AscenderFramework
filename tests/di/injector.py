...
class TestInjector(TestCase):
    ...
    def test_annotated_injection(self):
        ...
        root_injector = AscenderInjector(
            [{"provide": "settings", "value": "Some value"}, AnnotatedService]
        )
    ...
    def test_circular_injection(self):
        ...
        with self.assertRaises(
            CyclicDependency, msg="Circular dependency was not detected!"
        ):
    ...
    def test_hierarchy_injection(self):
        ...
        self.assertIsInstance(
            second_injector.get(ExampleService),
            ExampleService,
            "Second Injector failed to inject it's parent's token!",
        )

        # 3rd layered approach
        third_injector = AscenderInjector([], second_injector)
        self.assertIsInstance(
            third_injector.get(ExampleService),
            ExampleService,
            "Third Injector failed to inject it's parents' token! 3rd layered approach",
        )
    ...
    def test_multi_injection(self):
        ...
        class SomeService(Service):
            def __init__(
                self, some_values: Annotated[list[str], Inject("SUPER_VALUE")]
            ) -> None:
